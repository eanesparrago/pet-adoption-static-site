...
const RatingBlock = (props) => {
  ...
  const ratingPointTrail = useTrail(value, {
    from: { transform: "scale(1.4)", opacity: 0 },
    to: { transform: "scale(1)", opacity: 1 },
  });

  const ratingTextSpring = useSpring({
    from: { transform: "scale(1.4)", opacity: 0 },
    to: { transform: "scale(1)", opacity: 1 },
    delay: 1400,
  });
    ...
    <S.RatingBlock {...props}>
      ...
      <div className="RatingBlock__rating">
        <div className="RatingBlock__ratingPoint-group">
          ...
          <div className="RatingBlock__ratingPointOverlay-group">
            {ratingPointTrail.map((props, i) => (
              <animated.div
                style={props}
                className="RatingBlock__ratingPoint"
                key={i}
              >
                <svg
                  className="RatingBlock__ratingPointSvg"
        ...
        <animated.span
          style={ratingTextSpring}
          className="RatingBlock__ratingText"
        >
          {value > 0 && value}0%
        </animated.span>
      </div>
...
S.RatingBlock = styled.div`
  ...
  .RatingBlock__ratingPointOverlay-group {
    display: flex;
    position: absolute;
  }

  .RatingBlock__ratingText {
    font-size: 1.1875rem;
    font-weight: 700;
    color: ${(p) => p.theme.color.primary.main};
